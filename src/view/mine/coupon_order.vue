      ...
      <div class="infoO" v-for="item in dataList" :key="item.orderId">
        ...
        <div @click="goDetail(item.id)" class="info_item">
          <van-image
            ...
            width="100"
            height="100"
            src="https://qjz.oss-cn-shenzhen.aliyuncs.com/images/coupon.png"
          />
          <div class="info_itemR">
            <div>套餐: {{item.goodsName}}</div>
            <div>
              价格:
              <span class="price">{{item.orderPrice}}</span>
            </div>
            <div>1、提供15次一小时上门清洁服务</div>
            <div>2、追单第二个小时半价</div>
            <div class="con_bot">
              <div class="left">
  ...
  methods: {
    ...
    onCancel(item) {
      this.$dialog
        .confirm({
          title: '取消确认',
          message: '是否确认取消该优惠券订单，订单金额将原路返回',
        })
        .then(() => {
          axios
