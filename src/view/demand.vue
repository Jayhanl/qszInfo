  ...
  <div class="demand">
    ...
    <div class="demand_container demand_form">
      ...
      <van-form @submit="onSubmit">
        <van-field readonly clickable name="picker" :value="form.orderService" label="服务1" placeholder="点击选择服务1"
          :rules="[{ required: true, message: '请选择服务1' }]" @click="showDemand = true" />
        <van-popup v-model="showDemand" position="bottom">
          <van-picker show-toolbar :columns="columnsDemand" @confirm="onConfirm" @cancel="showDemand = false"
            @change="onChange" />
        </van-popup>
        <van-field readonly v-if="form.orderService" clickable name="picker" :value="form.orderService1" label="服务2"
          placeholder="点击选择服务2" @click="showDemand1 = true" />
        <van-popup v-model="showDemand1" position="bottom">
          <van-picker show-toolbar :columns="columnsDemand1" @confirm="onConfirm1" @cancel="showDemand1 = false"
            @change="onChange1" />
        </van-popup>
        <van-field v-model="form.orderExplain" rows="3" autosize label="详细说明" type="textarea" maxlength="300"
          placeholder="请输入详细说明..." show-word-limit :rules="[{ required: true, message: '请输入详细说明' }]" />
        <div class="tips">
          <!-- <p>受理时间： 10:30 - 22:30</p> -->
          <p>收到您的请求后，我们会尽快派遣服务人员联系您</p>
          <p>如果您有正在进行中的订单，请 <router-link :to="{name:'ordering'}">点击查看</router-link> </p>
        </div>
        <div class="btn ">
