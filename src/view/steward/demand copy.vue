<template>
  <van-form class="demand" @submit="onSubmit">
    <div class="demand_container">
      ...
      <div class="member">
        ...
        <span>管家到期时间: {{dataD.vipPastTime}}</span>
        <span>管家赠送服务剩余次数: {{dataD.vipServiceNum}}</span>
        <span>管家日免费服务剩余次数: {{dataD.vipFreeServiceNum}}</span>
      </div>
    </div>
    <div class="demand_container demand_form">
      ...
      <div class="tips">
        ...
        <p>
          如果您有正在进行中的订单，请
          <router-link :to="{name:'ordering'}">点击查看</router-link>
        </p>
      </div>
      <div class="btn">
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      if (
        this.dataD.vipServiceNum === 0 &&
        (this.form.orderNeed === 1 || this.form.orderNeed1 === 1)
      ) {
        this.$toast('本周管家赠送次数已用完,欢迎您下周再次使用~')
        return
      }
      if (this.form.orderNeed === 1 && this.form.orderNeed1 === 1) {
        this.$toast('不能同时选择两项管家赠送服务~')
        return
      }
      if (
        this.form.orderNeed === this.form.orderNeed1 &&
        this.form.orderService === this.form.orderService1
      ) {
      ...
      if (this.form.orderNeed === 2 || this.form.orderNeed1 === 2) {
        ...
        this.dataD.vipFreeServiceNum < 1 &&
        this.form.orderNeed !== 1
      ) {
